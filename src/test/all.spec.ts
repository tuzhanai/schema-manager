...
const schema4Json = require("./schema4.json");
...
const schema4 = {
  A: { type: "schema1", comment: "schema1" },
  B: { type: "schema2" },
  C: { type: "schema1[]" },
  D: { type: "schema2[]" },
};
...
describe("SchemaType", function () {
  ...
  it("required", function () {
    const { ok, message, missingParamaters, invalidParamaters } = Schema1.pick("stringP2").required().value({});
    expect(ok).to.equal(false);
  ...
  it("build swagger josn", function () {
    const { properties } = manager.get("schema4").swaggerValue();
    expect(properties).to.deep.equal(schema4Json);
  });
});
...
describe("SchemaManager", function () {
  ...
  it("type is schema[]", function () {
    ...
    expect(value).to.deep.equal({
      C: [
        { stringP1: "Hello", numP: 111 },
        { stringP1: "Hello", numP: 222 },
      ],
